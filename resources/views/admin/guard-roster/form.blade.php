    ...
    $(document).ready(function() {
        ...
        function initDatePicker(assignedDates, holidayDates, leaveDates) {
            const normalizedHolidays = holidayDates.map(holiday => ({
                name: holiday.name
                , date: moment(holiday.date).startOf('day').toDate() // Normalize holiday to start of the day in local time
            }));

            const normalizedLeaves = leaveDates.map(leave => ({
                date: moment(leave.date).startOf('day').toDate(), // Normalize leave to start of the day in local time
                status: leave.status
            }));
